...
async function scrapeInstagramReels(
  username,
  outputFile = `${username}_data.json`
) {
  ...
  try {
    ...
    if (fs.existsSync("cookies.json")) {
      ...
      if (!sessionValid) {
        ...
        fs.unlinkSync("cookies.json"); // Delete old cookies
    ...
    if (errorPageCheck) {
      console.log(
        "Error page detected, waiting for potential redirect to login page..."
      );
      await delay(10000);
    }
    ...
    if (!currentUrl.includes(`/${username}/`)) {
      ...
      throw new Error(
        "Failed to load profile page, possible login wall or block"
      );
    }
...
async function checkLoginWall(page) {
  ...
  const loginIndicators = ["button", 'input[name="username"]', "div"];
...
scrapeInstagramReels("larissa_wlc").catch((err) =>
