...
const InfluencerAnalysis = ({ data }: InfluencerAnalysisProps) => {
  ...
  const renderRedFlags = (flags: string[]) => {
    if (!flags || flags.length === 0 || (flags.length === 1 && flags[0] === 'N/A')) {
    ...
    <div className="relative w-full min-h-screen bg-black">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mt-8">
        <div className="border-b pb-4 mb-4">
          <h2 className="text-2xl font-bold text-indigo-800">{data['Influencer Name']} Analysis</h2>
          <div className="flex items-center text-gray-600 mt-1">
            <span className="mr-2">Instagram:</span>
            <span className="font-medium">{data['Social Media Handles'].Instagram}</span>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-indigo-50 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="bg-white px-2 py-1 rounded">{renderRating(data['Engagement Analysis'].Rating)}</div>
            </div>
            <p className="text-sm text-gray-700 mb-3">{data['Engagement Analysis'].Justification}</p>
            <div>
              <h4 className="text-xs font-semibold uppercase text-gray-500 mb-1">Red Flags</h4>
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="bg-white px-2 py-1 rounded">{renderRating(data['Influence Quality'].Rating)}</div>
            </div>
            <p className="text-sm text-gray-700 mb-3">{data['Influence Quality'].Justification}</p>
            <div>
              <h4 className="text-xs font-semibold uppercase text-gray-500 mb-1">Red Flags</h4>
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="bg-white px-2 py-1 rounded">{renderRating(data['Account Maturity'].Rating)}</div>
            </div>
            <p className="text-sm text-gray-700 mb-3">{data['Account Maturity'].Justification}</p>
            <div>
              <h4 className="text-xs font-semibold uppercase text-gray-500 mb-1">Red Flags</h4>
            ...
            <div>
              ...
              <ul className="list-disc pl-5 space-y-1">
                {data['Overall Assessment'].Concerns.length > 0 ? 
                  ...
                  : <li className="text-gray-700">No major concerns identified</li>
                }
              </ul>
            </div>
          </div>
