...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Check if username is provided in URL query params
    const usernameParam = searchParams.get("username")
    if (usernameParam) {
      setUsername(usernameParam)
      fetchAnalysisData(usernameParam)
    } else {
      // Check if there's any analysis data in sessionStorage
      const storedAnalysis = sessionStorage.getItem('profileAnalysis')
      if (storedAnalysis) {
        try {
          const parsedAnalysis = JSON.parse(storedAnalysis)
          setAnalysisData(parsedAnalysis)
          // If profile info exists, set the username
          if (parsedAnalysis.profileInfo && parsedAnalysis.profileInfo.username) {
  ...
  const fetchAnalysisData = async (usernameToFetch: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/scrape?username=${encodeURIComponent(usernameToFetch)}`, {
        method: 'GET',
      ...
      if (data.success && data.analysis) {
        ...
        setError(data.message || "Failed to get analysis data");
      }
    } catch (err: any) {
      console.error("Error fetching analysis:", err);
      
      if (err.name === 'AbortError') {
        setError("Request timed out. Analysis is taking too long, please try again.");
      } else if (err.message.includes('fetch') || err.message.includes('network')) {
        setError("Network error: Could not connect to the server. Please check your internet connection and try again.");
      } else {
        setError(`Error: ${err.message || "Unknown error occurred while fetching analysis data"}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Show input form if no analysis data is available
  if (!analysisData) {
    return (
      <DashboardShell>
        <DashboardHeader 
  ...
  // Render dashboard with analysis data
  return (
    <DashboardShell>
      <DashboardHeader 
        heading="Influencer Analytics" 
        text={`Comprehensive analysis and insights for @${username || (analysisData.profileInfo ? analysisData.profileInfo.username : "")}`}
      />
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card>
              ...
              <CardContent>
                ...
                <Progress 
                  value={parseInt((analysisData.profileInfo?.completeness?.percentage || "70%").replace("%", ""))} 
                  className="mt-2" 
                />
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Daily trend: {analysisData.ratings?.engagement?.dailyTrend || "±5%"}
                </p>
                <Progress 
                  value={parseInt((analysisData.ratings?.engagement?.percentage || "2.8%").replace("%", "")) * 10} 
                  className="mt-2" 
                />
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Brand Suitability</CardTitle>
                <Badge variant="outline">
              ...
              <CardContent>
                ...
                <Progress 
                  value={parseInt((analysisData.ratings?.brandSuitability?.alignment || "70%").replace("%", ""))} 
                  className="mt-2" 
                />
              </CardContent>
            </Card>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card className="col-span-1">
              ...
              <CardContent>
                <AreaChart data={analysisData.visualizationData?.engagementTrend} />
              </CardContent>
            </Card>
          </div>
        </TabsContent>
